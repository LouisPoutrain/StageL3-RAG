...
df_resultats = pd.read_excel(fichier_resultats, usecols=["Titre", "Évaluation d'invasivité"])
df_verite = pd.read_excel(fichier_verite, usecols=["Title", "Compliance with Taberlet"])
...
Ta tâche est de juger si la conclusions des deux lignes est la meme. ATTENTION Invasif=invasive et non-invasive=non-invasif. Si il y a une précision ignore la le plus important c'est que l'automatisation ai trouvé invasif ou non-invasif.

Réponds seulement par "Oui" ou "Non". Attention il ce peux que ce soit pas dans la meme langue mais tu ne dois pas prendre ça en compte et traduire. 

--- LIGNE EXPÉRIMENTALE ---
Titre : {ligne_exp['Titre']}
Conclusion : {ligne_exp["Évaluation d'invasivité"]}


--- LIGNE DE VÉRITÉ ---
Titre : {ligne_verite['Title']}
Conclusion : {ligne_verite['Compliance with Taberlet']}
...
df_final = pd.DataFrame({
    ...
    "Echantillon_exp3": df_resultats_matched["Évaluation d'invasivité"],
    "Titre_verite": df_verite_matched["Title"],
    "Echantillon_verite3": df_verite_matched['Compliance with Taberlet'],
    "score_titre": scores_titre,
    "verdict_llm3": verdicts
})

# Sauvegarde du fichier fusionné
df_final.to_excel("verdicts_llm3.xlsx", index=False)
