...
class RAGSystem:
    ...
    def index_from_json(self, json_path: str) -> int:
        ...
        # Créez une nouvelle instance de DocumentWriter pour la pipeline JSON
        json_writer = DocumentWriter(document_store=self.document_store)

        # Utilisez une pipeline pour traiter les documents JSON
        json_pipeline = Pipeline()
        json_pipeline.add_component("doc_embedder", SentenceTransformersDocumentEmbedder(model=self.embedder_model))
        json_pipeline.add_component("writer", json_writer)
        json_pipeline.connect("doc_embedder.documents", "writer.documents")

        # Exécutez la pipeline avec les documents JSON
        json_pipeline.run({"doc_embedder": {"documents": documents}})
    ...
    def index_documents(self, tei_folder: str = "tei") -> int:
       ...
       # Solution temporaire
        tei_files = Path("tei/Molecular.grobid.tei.xml") 
...
if __name__ == "__main__":
  ...
  #  num_docs = rag_system.index_documents("tei")
    rag_system.index_from_json("YOUR/PATH/file.json")
    
    #if num_docs > 0:
        # Question et définition
    question = "Dans l'article, peux-tu me donner tous les protocoles d'échantillonnage d'ADN et si ils sont considérés comme invasif ou non-invasif selon la définition de Taberlet ?"
