...
from rag.pipelines import create_hyde_pipeline, create_indexing_pipeline
from rag.components import DynamicThresholdAdapter
...
class RAGSystem:
    ...
    def __init__(self, api_key=None, api_url=None):
        ...
        if not self.api_url:
            raise ValueError("API URL must be provided or set as environment variables")
...
class RefineRAGSystem(RAGSystem):
    ...
    def refine_analysis(self, question: str, definition: str = "", top_k: int = 4, title: str = "") -> str:
        ...
        try:
            ...
            # Si aucun bloc explicite n'a été trouvé, tenter un chargement direct
            if not fused_protocols:
                try:
                    direct_json = json.loads(fused_protocols_response)
                    if isinstance(direct_json, dict):
                        direct_json = [direct_json]
                    if isinstance(direct_json, list):
                        for protocol in direct_json:
                            if isinstance(protocol, dict) and protocol.get("contient_protocole", False):
                                fused_protocols.append(protocol)
                except json.JSONDecodeError:
                    pass
        except Exception as e:
        ...
        # Fallback : si aucune donnée exploitable, tenter de parser directement les réponses initiales
        if not fused_protocols:
            fallback_protocols = []
            for protocol_str in protocols:
                try:
                    parsed_protocol = json.loads(protocol_str)
                    if isinstance(parsed_protocol, dict) and parsed_protocol.get("contient_protocole", False):
                        fallback_protocols.append(parsed_protocol)
                except json.JSONDecodeError:
                    continue
            fused_protocols = fallback_protocols
